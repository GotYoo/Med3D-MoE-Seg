...
class CTViT(nn.Module):
    ...
    def load(self, path):
        ...
        print(f"Loading weights from {path} with strict=False...")
        pt = torch.load(str(path), map_location='cpu')
        
        # 兼容性处理：如果权重包在 'state_dict' 里
        if 'state_dict' in pt:
            pt = pt['state_dict']
            
        # 【关键】开启非严格模式，允许缺失 Decoder 权重
        missing, unexpected = self.load_state_dict(pt, strict=False)
        
        # 打印日志让我们放心
        print(f"Weights loaded. Missing keys (expected for Encoder-only load): {len(missing)}")
        if len(unexpected) > 0:
            print(f"Unexpected keys (should be 0 now): {len(unexpected)}")
